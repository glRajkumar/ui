---
title: Select
description: A wrapper for Base UI Select with flexible option types, grouped options, size variants, backdrop, and custom value rendering.
---
...
```tsx
<SelectWrapper options={options} size="sm" placeholder="Small" />
<SelectWrapper options={options} size="default" placeholder="Default" />
```
...
type={`export interface SelectWrapper {
   ...
   * Receives the raw value string and the matched option (if found).
   *
   ...
   * Individual options can override with their own className.
   */
  itemCls?: string;

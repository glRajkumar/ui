---
title: Combobox
description: A wrapper for Base UI Combobox with single and multi-select, chips, grouped items, async loading, custom label rendering, and create-new support.
---
...
<ComboboxWrapper items={items} indicatorAt="" placeholder="No indicator" />
```
...
Used in `multiple` mode — controls what renders inside each chip. Receives the raw value string and the matched item:
...
Controls the string shown in the input after selection (single mode). Useful when item labels are ReactNodes:
...
type={`export interface ComboboxWrapper {
   ...
   * Receives the raw value string and the matched item (if any).
   *
   * If omitted, the item label (ReactNode) is used directly.
   *
   * \`\`\`tsx
